...
export const CaseStudyEngine: React.FC<{ question: Question }> = ({ question }) => {
  ...
  const tabs = [
    ...
    { id: 'business', label: 'Business', icon: Building2, title: 'Business Requirements', content: cs.businessRequirements },
    { id: 'technical', label: 'Technical', icon: Cpu, title: 'Technical Requirements', content: cs.technicalRequirements },
    { id: 'existing', label: 'Environment', icon: Server, title: 'Existing Environment', content: cs.existingEnvironment },
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
        ...
        <div className="bg-white rounded border border-slate-300 shadow-sm flex flex-col min-h-[500px]">
          {/* Scenario Tab Header: Flex-wrap grid ensuring 100% visibility of all tabs without cutoff */}
          <div className="bg-slate-100 p-2 border-b border-slate-300 flex flex-wrap gap-1.5 rounded-t">
            {tabs.map((tab) => {
              const Icon = tab.icon;
              ...
              return (
                <button
                  ...
                  className={`flex-1 min-w-[85px] sm:min-w-[100px] flex items-center justify-center gap-1.5 px-3 py-2 text-xs sm:text-sm font-extrabold rounded transition-all border shadow-xs ${
                    isActive
                      ? 'bg-ntms-navy text-white border-ntms-darkNavy shadow-sm ring-1 ring-sky-400'
                      : 'bg-white text-slate-700 border-slate-300 hover:bg-slate-200/80 hover:text-ntms-navy'
                  }`}
                >
                  <Icon className={`w-3.5 h-3.5 ${isActive ? 'text-sky-300' : 'text-sky-600'}`} />
